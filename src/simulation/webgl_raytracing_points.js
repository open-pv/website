...
export function rayTracingPointsWebGL(
  ...
  const vertexShaderSource = `#version 300 es
	...
	bool Calculate_Shading_at_Point_Triangles(vec3 vertex_position, vec3 sun_direction) {
		float d;
		float t = INFINITY;
	...
	bool Calculate_Shading_at_Point(vec3 vertex_position, vec3 sun_direction) {
        ...
        for (int i = 0; i < MAX_STEPS; i++) {
            for (int j = 0; j < textureDepth; j++) {
                ...
                // if (dot(pointPos - vertex_position, pointPos - vertex_position) < 200.){
                //     return true;
                // }
                // if ((abs(pointPos.x - vertex_position.x) < 10.)&&(abs(pointPos.y - vertex_position.y) < 10.)){
                //     return true;
                // }
            ...
            if ((textureWidth <= gridPos.x) || (textureDepth <= gridPos.y) || (gridPos.x < 0) || (gridPos.y < 0)){
                break;
            }
        }
	...
	void main() {
		if (Calculate_Shading_at_Point(a_position.xyz, u_sun_direction) || Calculate_Shading_at_Point_Triangles(a_position.xyz, u_sun_direction)) {
					outColor = vec4(0, 0, 0, 0); // Shadowed
        } else {
            float intensity = dot(a_normal.xyz, u_sun_direction);
