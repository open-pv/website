...
function handleSpaceKey() {
  ...
  if (intersects.length > 0) {
    ...
    const offsetPoint = intersect.face
      ? intersect.point
          .clone()
          .add(intersect.face.normal.clone().multiplyScalar(0.1))
    ...
    console.log(
      ...
      pointColors[pointColors.length - 1].getStyle()
    )
...
function subdivideTriangle(triangle, threshold) {
  ...
  if (ab > threshold || bc > threshold || ca > threshold) {
    ...
    if (ab >= bc && ab >= ca) {
      longestEdgeMidpoint = triangle.a
        .clone()
        .add(triangle.b)
        .multiplyScalar(0.5)
      return [
        ...subdivideTriangle(
          { a: triangle.a, b: longestEdgeMidpoint, c: triangle.c },
          threshold
        ),
        ...subdivideTriangle(
          { a: longestEdgeMidpoint, b: triangle.b, c: triangle.c },
          threshold
        ),
      ]
    } else if (bc >= ab && bc >= ca) {
      longestEdgeMidpoint = triangle.b
        .clone()
        .add(triangle.c)
        .multiplyScalar(0.5)
      return [
        ...subdivideTriangle(
          { a: triangle.a, b: triangle.b, c: longestEdgeMidpoint },
          threshold
        ),
        ...subdivideTriangle(
          { a: triangle.a, b: longestEdgeMidpoint, c: triangle.c },
      ...
      longestEdgeMidpoint = triangle.c
        .clone()
        .add(triangle.a)
        .multiplyScalar(0.5)
      return [
...
function calculatePolygonIntensity(vertices, intensities) {
  ...
  for (let i = 0; i < numTriangles; i++) {
    ...
    console.log("triangleIntensity", triangleIntensity)
  }
...
function filterPolygonsByDistance(scene, points, threshold) {
  ...
  scene.traverse((child) => {
    if (child.isMesh && child.name === "simulationMesh") {
